## Results
Here we describe the the functionality and interface for the visualizations and explain why they are useful for analyzing electrophysiological data.
...
The network layout toggle controls where nodes are positioned (and consequently the edges between nodes). Because the networks are dynamic and may change based on time or frequency, `SpectraVis` offers a *topological* layout --- which uses a force-directed layout to limit the number of overlapping edges. There is also an *anatomical* layout which places the nodes according to the anatomical position defined in the JSON files.
...
#### `GLMVis`
`GLMVis` the generalized linear models that: (1) shows the relationship between the multiple dimensions of the model fit over time, (2) shows the relationship between multiple models, and (3) show the relationship between multiple brain areas. To show the relationship between multiple dimensions, we  use the parallel coordinate plots [@wegman_hyperdimensional_1990].
